...
Assessment status 流转：`pending` → `analyzed`（scoring engine 成功后）→ `complete`（report writer 成功后）
...
**D4 归一化（⚠️ scoring engine 必须执行，否则 T3 用户被系统性低估）：**
...
### report writer 流式输出格式

report writer 输出**强制带 `--Section--` 标记的纯文本**（不是 JSON），后端 `app/api/ws_result.py` 的 `_SectionStreamer` 实时检测标记并通过 WebSocket 发送结构化消息：
